...
c
      integer key
      double precision sxx,syy,szz,sxy,syz,szx,p,f,st0,di0,ra0
c
c     output
c     cmb = max. Coulomb stress, and
...
c
      double precision cmb,sig,st1,di1,ra1,st2,di2,ra2
c
c     local memories:
c
      integer i,j
      double precision pi,rad2deg,b,c,d,s1,s2,s3
      double precision alpha,am,swap,det1,det2,det3
      double precision s(3),st(2),di(2),ra(2),rst(3),rdi(3)
      double precision r(3,2),ns(3,2),ts(3,2)
...
c
      ...
      cmb=0.5d0*(s1-s2)*dsqrt(1+f*f)+f*(0.5d0*(s1+s2)+p)
c
      if(key.eq.0.or.dabs(s1-s2).le.1.0d-08*dsqrt(s1**2+s2**2))then
c
...
c
      do j=1,2
        ...
        if(dabs(det1).ge.dmax1(dabs(det2),dabs(det3)))then
          r(1,j)=det1
          r(2,j)=(szz-s(j))*sxy-syz*szx
          r(3,j)=(syy-s(j))*szx-syz*sxy
        else if(dabs(det2).ge.dmax1(dabs(det1),dabs(det3)))then
